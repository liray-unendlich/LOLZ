...
void Shutdown()
{
    ...
    RenameThread("loanorleasez-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: loanorleasez.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: loanorleasezd.pid)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 23233)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeloanorleasezamount=<n> " + _("Keep N LoanOrLeaseZ anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. LoanOrLeaseZ is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. LoanOrLeaseZ is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("LoanOrLeaseZ version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "LoanOrLeaseZ server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of LoanOrLeaseZ") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart LoanOrLeaseZ to complete") << "\n";
    ...
    nAnonymizeloanorleasezAmount = GetArg("-anonymizeloanorleasezamount", 0);
    if(nAnonymizeloanorleasezAmount > 999999) nAnonymizeloanorleasezAmount = 999999;
    if(nAnonymizeloanorleasezAmount < 2) nAnonymizeloanorleasezAmount = 2;
    ...
    LogPrintf("Anonymize LoanOrLeaseZ Amount %d\n", nAnonymizeloanorleasezAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1LOLZ+1000 == (.1LOLZ+100)*10
       10LOLZ+10000 == (1LOLZ+1000)*10
    */
