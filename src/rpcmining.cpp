...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "LoanOrLeaseZ is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "LoanOrLeaseZ is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "LoanOrLeaseZ is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "LoanOrLeaseZ is downloading blocks...");
