...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeloanorleasezAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeloanorleasezAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeloanorleasezAmount = strAnonymizeloanorleasezAmount.remove(strAnonymizeloanorleasezAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeloanorleasezAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeloanorleasezAmount*COIN) nMaxToAnonymize = nAnonymizeloanorleasezAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeloanorleasezAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeloanorleasezAmount));
        strAnonymizeloanorleasezAmount = strAnonymizeloanorleasezAmount.remove(strAnonymizeloanorleasezAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeloanorleasezAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeloanorleasezAmount == 0){
