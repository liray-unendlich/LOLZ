...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("loanorleasez"))
        return false;
...
{
    // Convert loanorleasez:// to loanorleasez:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("loanorleasez://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "loanorleasez:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LoanOrLeaseZ.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for LoanOrLeaseZ.lnk
...
{
    return GetAutostartDir() / "loanorleasez.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a loanorleasez.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LoanOrLeaseZ\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the loanorleasez app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add loanorleasez app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LoanOrLeaseZ-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  loanorleasez-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("LoanOrLeaseZ-Qt"));
