    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat LoanOrLeaseZ and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002LOLZ per transaction on average.
            */
            if(r <= 10)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeloanorleasezAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100LOLZ+1 ( bit on if present )
    // bit 1 - 10LOLZ+1
    // bit 2 - 1LOLZ+1
    // bit 3 - .1LOLZ+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("loanorleasez-darksend");
